...
class PoguesImplTest {
    ...
    @Test
    void getOperationsWithPeriodicityBySerieIdTest() throws IOException, RmesException {
        JSONArray mockJSON = new JSONArray(PoguesUtilsTest.OPERATION_WITH_PERIODICIY_BY_SERIE);
        when(repoGestion.getResponseAsArray(Mockito.anyString())).thenReturn(mockJSON);
        assertThat(MAPPER.readTree(poguesImpl.getOperationsBySerieId("1"))).isEqualTo(MAPPER.readTree(PoguesUtilsTest.EXPECTED_OPERATION_WITH_PERIODICIY_BY_SERIE.toString()));
    }
