...
public class UserDecoderConfiguration {
    ...
    @Bean
    public fr.insee.rmes.magma.security.UserDecoder userDecoder() {
        return principal -> "anonymousUser".equals(principal) ?
                empty() :
                of(buildUserFromToken(((Jwt) principal).getClaims()));
    }

    protected fr.insee.rmes.magma.security.User buildUserFromToken(Map<String, Object> claims) throws RmesException {
        if (claims.isEmpty()) {
        ...
        return new fr.insee.rmes.magma.security.User(id, email);
    }
